...
# Group by metrics.
df_game_follow = df.groupby(["genre_name"])["follow_count"].sum().reset_index()

df_genre_follow = df_genre_follow.rename(columns = {"follow_count": "total_count"})

df_genre_hype = df.groupby(["genre_name"])["hype_count"].sum().reset_index()

df_genre_hype = df_genre_hype.rename(columns = {"hype_count": "total_count"})
