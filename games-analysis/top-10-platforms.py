...
# Group by metrics.
df_game_follow = df.groupby(["platform_name"])["follow_count"].sum().reset_index()

df_platform_follow = df_platform_follow.rename(columns = {"follow_count": "total_count"})

df_platform_hype = df.groupby(["platform_name"])["hype_count"].sum().reset_index()

df_platform_hype = df_platform_hype.rename(columns = {"hype_count": "total_count"})
